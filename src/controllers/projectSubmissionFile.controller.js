...
const getAllFiles = asyncHandler(async (req, res) => {
  const files = await ProjectSubmissionFile.find({
    owner: req.user._id,
  });
...
const uploadFile = asyncHandler(async (req, res) => {
  ...
  const uploadedFile = await ProjectSubmissionFile.findById(file._id);
...
const deleteFile = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const result = await ProjectSubmissionFile.deleteOne({
    _id: id,
  });
