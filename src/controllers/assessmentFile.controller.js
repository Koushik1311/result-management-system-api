...
const getAllFiles = asyncHandler(async (req, res) => {
  const files = await AssessmentFile.find({
    owner: req.user._id,
  });
...
const uploadFile = asyncHandler(async (req, res) => {
  ...
  const file = await AssessmentFile.create({
    fileUrl: response.url,
    isExtracted: false,
    owner: req.user._id,
  });

  const uploadedFile = await AssessmentFile.findById(file._id);
...
const deleteFile = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const result = await AssessmentFile.deleteOne({
    _id: id,
  });
