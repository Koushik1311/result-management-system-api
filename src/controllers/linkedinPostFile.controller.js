...
const getAllFiles = asyncHandler(async (req, res) => {
  const files = await LinkedinPostFile.find({
    owner: req.user._id,
  });
...
const uploadFile = asyncHandler(async (req, res) => {
  ...
  const file = await LinkedinPostFile.create({
    fileUrl: response.url,
    isExtracted: false,
    owner: req.user._id,
  });

  const uploadedFile = await LinkedinPostFile.findById(file._id);
...
const deleteFile = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const result = await LinkedinPostFile.deleteOne({
    _id: id,
  });
